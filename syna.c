...
/* These are global */
int         syna_counter,syna_row;

static float   *aalto[WAVES],*instr[INSTR],off[INSTR],plus[INSTR],
               *echo[INSTR],pan[INSTR],prev[INSTR],pola[INSTR],vol[INSTR],
               slide[INSTR];
static int  global,slen,update,nupdate,trak[INSTR][PTLEN],ptn[MAXR][PTLEN],
            ti[INSTR],pi[INSTR],len[INSTR],ekolen,firstrow,transp[INSTR];
static char *module,eko[INSTR],mute[INSTR],nextmute[INSTR];
...
{
    int     n=0,i,j,tmp,a,d,s,r,mod,sweep,wave,patt,track,note;
...
void syna_play(short *dest,int length)
{
    ...
    for(n=0;n<length;n++,syna_counter++)
    {
        /* New row */
        if(syna_counter%update==0)
        {
            if(firstrow)
                firstrow=0;
            else
            ...
            for(i=1;trak[i][0]!=END;i++)
            {
                ...
                if(ti[i]!=END)
                {
                    note=ptn[trak[i][ti[i]]][pi[i]];
                    switch(note)
                    {
                        case NEMPTY: break;
                        case NSTOP : off[i]=-1.0; break;
                        case NECHON: eko[i]=1; break;
                        case NECHOFF: eko[i]=0; break;
                        case NVOL  : pi[i]++;
                        ...
                        case NSLIDE: pi[i]++;
                                     ...
                                     break;
                        default    : note+=transp[i];
                                     if(note<Nc1) note=Nc1;
                                     if(note>Nb6) note=Nb6;
                                     plus[i]=notei[note];
                                     plus[i]/=(float)BFREQ;
                                     off[i]=0.0;
                    }
        ...
        for(i=1;trak[i][0]!=END;i++)
        {
            ...
            smp=echo[i][(syna_counter+1)%ekolen];

            echo[i][syna_counter%ekolen]=echo[i][(syna_counter+1)%ekolen]*0.6;
            if(off[i]>=0.0)
            {
                smp+=instr[i][(int)off[i]];

                if(eko[i])
                    echo[i][syna_counter%ekolen]=smp*0.2;
...
{
    if(chn<INSTR && trak[chn][0]!=END)
        vol[chn]=nvol;
}

void syna_setbpm(int bpm)
{
    nupdate=6*global/(bpm*10/25);
}

void syna_trig(int chn,int note)
{
    if(chn<INSTR && trak[chn][0]!=END && note<=Nb6)
    {
        plus[chn]=notei[note];
        plus[chn]/=(float)BFREQ;
        off[chn]=0.0;
    }
}
